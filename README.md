...
Repository navigation splits into two separately registered MCP servers. The
custom `code-nav` MCP (`code_nav_server.py`) is **only a repository-map router**:
its single tool `mcp__code-nav__repository_route` selects child maps and paths
from `docs/design/repository_map.yaml`, nothing more — it no longer does literal
searches, symbol indexes, or environment checks. **Serena** owns semantic code
navigation (project activation, symbol overview, symbol/declaration/reference
lookup, diagnostics, bounded pattern search, and reading existing memories)
and is granted to read-only delegates as the exact read-only tool set
(`server.SERENA_READ_ONLY_MCP_TOOLS`); onboarding and all source/memory mutation
tools are excluded. Keeping both separate from the delegation server matters: delegates
