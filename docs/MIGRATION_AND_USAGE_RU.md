...
#      get_delegate_result(<run_id>, wait_seconds=900)  # ждёт и возвращает 'ok'
...
- **Обычная делегация без polling:** после запуска вызвать
  `get_delegate_result(run_id, wait_seconds=900)`. MCP ждёт внутри одного вызова и
  сразу возвращает компактный финал; если двух минут не хватило — повторить вызов.
  `check_delegate_status` / `watch_delegate` использовать только для диагностики
  зависания или ухода не туда. Каждый обычный polling возвращает управление
  основной модели и создаёт ещё один платный ход со всем накопленным контекстом.
  Для `delegate_verified` пока нужен `check_verified_status`: его вызовы двигают
  state machine worker → checker → revise.
...
Для обычной работы супервизора сначала читайте короткий
`ARCHITECT_BRIEF.md`; полные `COORDINATION.md` и `TASK_DESIGN.md` нужны при
конфликтах, восстановлении и подготовке сложной делегации. Историческую сводку
можно пересчитать командой `python3 contrib/history_stats.py --pretty`. Скрипт
потоково читает локальные JSONL и выводит только агрегаты: длительность, число
ходов, токены, пиковый контекст, размер финала и частоты инструментов. Тексты
промптов и ответов, пути транскриптов и секреты в отчёт не попадают.
...
- **Сжатие результатов.** `get_delegate_result` по умолчанию возвращает первые 4 +
  последние 15 строк финального ответа и sha256 полного текста (вместо всего
  ...
  `CLAUDE_LOCAL_DELEGATE_RESULT_LINES` (15), `CLAUDE_LOCAL_DELEGATE_FANOUT_LINES` (15).
