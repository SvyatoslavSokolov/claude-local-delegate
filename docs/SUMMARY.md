...
- [`cluster_telemetry.py`](../cluster_telemetry.py)
  - **Назначение:** сбор живой телеметрии с кластера 4x RTX 3090 через Prometheus-метрики vLLM и шлюз LiteLLM (число активных запросов, очередь/конкуренция, утилизация KV-кэша, хит-рейт префиксного кэша).
- [`dashboard.py`](../dashboard.py)
  - **Назначение:** веб-дашборд и REST API на чистом stdlib HTTP (`http://127.0.0.1:8765`), отображающий в реальном времени канбан-доску задач, матрицу качества и ROI, статистику вызовов инструментов и нагрузку GPU кластера.
